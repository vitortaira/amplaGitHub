...
ig_vcxve <- 
  ...
  vcxve_gdf <- 
    data.frame(
      ...
      valor = 
        c(dados.ciweb.relatorio_l[[empreendimento.c]][[t]][[i]]$Emprestimo$`Valor Empréstimo` - 
            dados.ciweb.relatorio_l[[empreendimento.c]][[t]][[i]]$Emprestimo$`Valor Reduzido`,
          dados.ciweb.relatorio_l[[empreendimento.c]][[t]][[i]]$Emprestimo$`Valor Reduzido`,
          sum(dados.ciweb.relatorio_l[[empreendimento.c]][[t]][[i]]$Unidades$`Valor Liberado Terreno`),
          sum(dados.ciweb.relatorio_l[[empreendimento.c]][[t]][[i]]$Unidades$`Valor Liberado Obra`))
    ) %>%
    group_by(barra) %>%
