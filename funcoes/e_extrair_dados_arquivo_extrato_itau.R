...
# extrair_dados_arquivo_extrato_cef() extrai os dados de um arquivo de extrato
# da CEF em PDF.
...
# f_caminho.arquivo.extrato.cef_c: String do caminho de um arquivo de extrato
# da CEF em PDF.
...
extrair_dados_arquivo_extrato_cef <- 
  function(f_caminho.arquivo.extrato_cef_c) {
    ...
    if (paginas_l[[1]] %>% str_detect("Extra([A-Za-z]{2})? por per") %>% sum > 0) {
      # Define linhas_c
      linhas_c <- 
      ...
      data.consulta_h <-
        if_else(
          linhas_c %>% nth(1) %>% str_starts("\\d{2}/\\d{2}/\\d{4}"),
          linhas_c %>%
            nth(1) %>% 
          ...
          if_else(
            linhas_c %>% str_starts("Data:") %>% sum > 0,
            (linhas_c %>% 
      ...
      #  )
      # Índices
      indice.comeco_i <- linhas_c %>% str_which("^\\d{2}") %>% nth(1)
      indice.fim_i <- linhas_c %>% str_which("^\\d{2}/\\d{2}/\\d{4}") %>% last
      # Define extrato_t
      extrato_t <- 
        linhas_c %>%
    ...
    # Se o extrato da CEF for do tipo sem o título "Extrato por período"
      ...
      agencia_c <- 
        ...
        str_sub(-4, -1)
      cliente_c <- linhas_c %>% nth(1) %>% str_trim
      cnpj_c <- 
        linhas_c %>% 
        nth(2) %>% 
        str_remove("^CNPJ:\\s*") %>% 
...
f_caminho.arquivo.extrato_cef_c <- caminhos.extratos.cef_c[14]
