...
cruzar_extrato_cmfcn <- 
  function(
    ...
    caminhos.extratos.cef_c <- 
      list.files(
        f_caminho.pasta.extratos_c, full.names = TRUE, recursive = T
      ) %>%
      keep(
        ~ str_ends(.x, ".pdf")
    ...
    for (
      ...
      extratos_l[[i_caminho.extrato.cef_c]] <- 
        extrair_dados_arquivo_extrato_cef(i_caminho.extrato.cef_c)
      extratos_t <- 
    ...
    for (
      ...
      cmfcns_l[[i_caminho.cmfcn_c]] <- 
        extrair_dados_arquivo_cmfcn(i_caminho.cmfcn_c)
      cmfcns_t <- 
