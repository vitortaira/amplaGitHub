...
gerar_xlsx <- function(data,
  ...
  for (i in seq_along(dados_lista)) {
    ...
    if (is.null(wb_load)) {
      ...
      # Estilo para valores monetários (baseado no argumento 'col_monetary')
      ...
      # Estilo para datas (baseado no tipo da coluna)
      colunas_data <- which(sapply(df_dados, function(x) inherits(x, "Date")))
